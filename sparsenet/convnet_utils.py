...
class NeuralNetLayerCfg(object):
    def __init__(self, dropout, lifetime_sparsity, spatial_sparsity, l2, want_train = True):
        self.type = None
...
class NeuralNetLayerDenseCfg(NeuralNetLayerCfg):
    def __init__(self,num_filters,activation, initW, initB, dropout, lifetime_sparsity, spatial_sparsity, l2):
        NeuralNetLayerCfg.__init__(self,dropout, lifetime_sparsity, spatial_sparsity, l2)
        self.type = "dense"
...
class NeuralNetLayerConvolutionCfg(NeuralNetLayerCfg):
    def __init__(self, num_filters, activation, filter_width, stride, padding, initW, initB, dropout, l2, lifetime_sparsity, spatial_sparsity, partial_sum  = 1):
        NeuralNetLayerCfg.__init__(self,dropout, lifetime_sparsity, spatial_sparsity, l2)
    ...
    def applyConvDown(self, delta, w, imSizeX):
        return nn.ConvDown(delta, w, moduleStride = self.stride, paddingStart = self.padding, imSizeX = imSizeX)
...
class NeuralNetLayerDeConvolutionCfg(NeuralNetLayerCfg):
    def __init__(self, num_filters, activation, filter_width, stride, padding, initW, initB, dropout, l2, lifetime_sparsity, spatial_sparsity, partial_sum  = 1):
        NeuralNetLayerCfg.__init__(self,dropout, lifetime_sparsity, spatial_sparsity, l2)
    ...
    def fprop(self, H_prev, w, b, H, dH):
            H = nn.DeConvUp(H_prev, w, moduleStride = self.stride, paddingStart = self.padding, imSizeX = self.imSizeX)        
            H += b.reshape(1,-1,1,1)   
            self.activation(H,H,dH) 
            return H,dH

    def applyDeConvDown(self, delta, w):
        return nn.DeConvDown(delta, w, moduleStride = self.stride, paddingStart = self.padding)
    def applyDeConvOut(self, H, delta):
        return nn.DeConvOut(H, delta, moduleStride = self.stride, paddingStart = self.padding, partial_sum = self.partial_sum, filterSizeX = self.filter_shape[2])     


class NeuralNetLayerResponseNormCfg(NeuralNetLayerCfg):
    def __init__(self, N, addScale, powScale, blocked = 1, minDiv = 1):
        NeuralNetLayerCfg.__init__(self, dropout=None, lifetime_sparsity=None, spatial_sparsity=None, l2=None)
        self.type = "reparam"
        self.N = N
...
class NeuralNetCfg(object):
    def __init__(self, want_dropout=False,want_lifetime_sparsity = False, want_spatial_sparsity = False,
                 want_tied=False, tied_list=[(1,2)],num_groups = None, want_layerwise_train = False, want_extra = False):
        self.name = "No-Name"
        ...
        self.want_lifetime_sparsity = want_lifetime_sparsity
        self.want_spatial_sparsity = want_spatial_sparsity
        self.num_groups = num_groups
        self.want_layerwise_train = want_layerwise_train
        self.want_extra = want_extra
            
    def input_conv(self,shape, dropout=None):
        layer = NeuralNetLayerConvolutionCfg(num_filters=None, activation=None, filter_width=None, stride=None, padding=None, initW=None, initB=None, dropout=dropout, l2=None, lifetime_sparsity = None, spatial_sparsity = None)
        layer.shape = shape
        layer.size = shape[0]*shape[1]*shape[2]
        self._layers[0] = layer

    def input_dense(self,shape, dropout=None,extra=None):
        layer = NeuralNetLayerDenseCfg(num_filters=None, activation=None, initW=None, initB=None, dropout=dropout, lifetime_sparsity=None, spatial_sparsity=None, l2=None)
        layer.extra = extra
        layer.shape = shape
        layer.size = shape
        self._layers[0] = layer
    
    def convolution(self, num_filters, activation, filter_width=None, stride=1, padding=0, initW = None, initB = None, dropout = None, 
                    lifetime_sparsity=None, spatial_sparsity=None, l2=None, partial_sum = 1, want_train = True, extra=None):
        layer = NeuralNetLayerConvolutionCfg(num_filters=num_filters, activation=activation, filter_width=filter_width, stride=stride, padding=padding, 
                                             initW=initW, initB=initB, dropout=dropout, lifetime_sparsity=lifetime_sparsity, spatial_sparsity=spatial_sparsity, l2=l2, 
                                             partial_sum = partial_sum)      
        layer.extra = extra        
        layer.want_train = want_train        
        self._layers.insert(-1,layer)

    def deconvolution(self, num_filters, activation, filter_width=None, stride=1, padding=0, initW = None, initB = None, dropout = None, 
                      lifetime_sparsity=None, spatial_sparsity=None, l2=None, partial_sum = 1, want_train = True, imSizeX = "auto"):
        
        layer = NeuralNetLayerDeConvolutionCfg(num_filters=num_filters, activation=activation, filter_width=filter_width, stride=stride, padding=padding, 
                                                initW=initW, initB=initB, dropout=dropout, lifetime_sparsity=lifetime_sparsity, spatial_sparsity=spatial_sparsity, l2=l2, partial_sum = partial_sum)      
        layer.want_train = want_train  
        self.imSizeX = imSizeX                   
        self._layers.insert(-1,layer)
    ...
    def dense(self, num_filters, activation, 
            initW = None, initB = None, dropout = None, lifetime_sparsity = None, spatial_sparsity=None, 
            l2 = None, want_train=True, l2_activity = None, extra=None):
        layer = NeuralNetLayerDenseCfg(num_filters=num_filters, activation=activation, initW=initW, initB=initB, dropout=dropout, lifetime_sparsity=lifetime_sparsity, spatial_sparsity=spatial_sparsity, l2=l2)
        layer.extra = extra
        layer.want_train = want_train   
        layer.l2_activity = l2_activity     
        self._layers.insert(-1,layer)
    ...
    def output_dense(self, num_filters, activation, initW = None, initB = None, dropout = None, l2 = None, want_train=True,extra=None):
        layer = NeuralNetLayerDenseCfg(num_filters=num_filters, activation=activation, initW=initW, initB=initB, dropout=dropout, lifetime_sparsity = None, spatial_sparsity = None, l2=l2)
        layer.want_train = want_train 
        layer.extra=None                      
    ...
    def output_conv(self, num_filters, activation, filter_width=None, stride=1, padding=0, initW = None, initB = None, l2=None, partial_sum = 1):
        layer = NeuralNetLayerConvolutionCfg(num_filters=num_filters, activation=activation, filter_width=filter_width, stride=stride, padding=padding, initW=initW, initB=initB, dropout=None, lifetime_sparsity=None, spatial_sparsity=None, l2=l2, partial_sum = partial_sum)
        self._layers[-1] = layer
        # self.finalize()    

    def output_deconv(self, num_filters, activation, filter_width=None, stride=1, padding=0, initW = None, initB = None, l2=None, partial_sum = 1, imSizeX = "auto"):
        layer = NeuralNetLayerDeConvolutionCfg(num_filters=num_filters, activation=activation, filter_width=filter_width, stride=stride, padding=padding, initW=initW, initB=initB, dropout=None, lifetime_sparsity=None, spatial_sparsity=None, l2=l2, partial_sum = partial_sum)
        layer.imSizeX = imSizeX      
        self._layers[-1] = layer
        # self.finalize()          
    ...
    def params(self,arch,learning,dataset,dataset_extra=None,test_only = False):
        self.arch = arch
    ...
    def finalize(self):
        for k in range(1,len(self)):
            ...
            if self[k].type == "convolution":
                layer.shape = [ layer.num_filters,
                                (layer_previous.shape[1]+2*layer.padding-layer.filter_width)/layer.stride+1,
                                (layer_previous.shape[2]+2*layer.padding-layer.filter_width)/layer.stride+1   ]
                layer.filter_shape = [layer.shape[0],
                                      layer_previous.shape[0],
                                      layer.filter_width,layer.filter_width]

                layer.weights_shape = [  [layer.shape[0],layer_previous.shape[0],layer.filter_width,layer.filter_width],
                                         [1,layer.num_filters]   ]
                layer.size = layer.shape[0] * layer.shape[1] * layer.shape[2]

            if self[k].type == "deconvolution":
                imSizeX_temp = (layer_previous.shape[1]-1)*layer.stride-2*layer.padding+layer.filter_width
                if layer.imSizeX == "auto":
                    layer.imSizeX = imSizeX_temp
                else:
                    assert (layer.imSizeX - imSizeX_temp)/layer.stride == 0

                layer.shape = [ layer.num_filters,
                                layer.imSizeX,
                                layer.imSizeX   ]
                # (numModulesX - 1) * moduleStride - 2*abs(paddingStart) + filterSizeX

                layer.filter_shape = [layer_previous.shape[0],
                                      layer.shape[0],
                                      layer.filter_width,layer.filter_width]

                layer.weights_shape = [  [layer_previous.shape[0],layer.shape[0],layer.filter_width,layer.filter_width],
                                         [1,layer.num_filters]   ]
                layer.size = layer.shape[0] * layer.shape[1] * layer.shape[2]

            if self[k].type == "dense":
                layer.weights_shape = [[layer_previous.size,layer.num_filters],[1,layer.num_filters]]         
                layer.filter_shape = layer_previous.size

            if self[k].type == "pooling":
                # print layer_previous.shape[1],layer.stride,int(np.ceil(1.0*layer_previous.shape[1]/layer.stride))
                layer.shape = [ layer_previous.shape[0],
                                int(np.ceil(1.0*layer_previous.shape[1]/layer.stride)), 
                                int(np.ceil(1.0*layer_previous.shape[2]/layer.stride))]
                layer.size = layer.shape[1] * layer.shape[2] * layer.shape[0]
                layer.weights_shape = [None,None]

            if self[k].type == "rnorm":
                layer.size = layer.shape = layer_previous.size     
                layer.weights_shape = [None,None]

            if self[k].type == "reparam":
