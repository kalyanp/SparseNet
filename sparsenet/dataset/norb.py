...
class NORB():
    @staticmethod
    def load(size=64,want_mean=False,want_dense=False):

        X = np.zeros((24300,size**2))        
        ...
        if size==64:
            X = data[:,0,16:80,16:80].reshape(24300,64**2)
        else: 
...
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!numpy object dtype correct it
    def load_norb(backend,dtype,resize=64,mode="single"):
        ...
        data = open(work_address+'./Dataset/NORB/smallnorb-5x46789x9x18x6x2x96x96-training-cat.mat','r')
        data = data.read()
        data = np.fromstring(data[20:], dtype='uint32')
        ...
        data_ = open(work_address+'./Dataset/NORB/smallnorb-5x46789x9x18x6x2x96x96-training-dat.mat','r')
        data_ = data_.read() 
        ...
        data=open(work_address+'./Dataset/NORB/smallnorb-5x01235x9x18x6x2x96x96-testing-cat.mat','r')
        data=data.read() 
        data=np.fromstring(data[20:], dtype='uint32')
        T_labels=data
        if (mode == "single" or mode == "parallel" or mode == "binocular"): 
        ...
        data_=open(work_address+'./Dataset/NORB/smallnorb-5x01235x9x18x6x2x96x96-testing-dat.mat','r')
        data_=data_.read() 
        data_=np.fromstring(data_[24:], dtype='uint8')
        data_=np.reshape(data_,(24300,2,96,96))
        data=data_[:,0,16:80,16:80]
